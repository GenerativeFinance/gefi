...
| [x] | 109 | Sign in / Sign up (prelaunch state) | Auth & operator surfaces | Medium priority | Done. Muted product-preview strip (3 KPI tiles + chart silhouette, aria-hidden, caption "What opens up when sign-in goes live") under the prelaunch card in _layouts/auth.html. /register/ has since become a full intake form page, so the strip applies to the auth card (/login/); register untouched. Build + Playwright verified. |
| [x] | 110 | Admin sign-in | Auth & operator surfaces | Medium priority | Done. /admin/ operator card: slate palette + amber accent, "Internal preview" corner ribbon, Cloudflare Access caveat moved to persistent footer strip, admin-gate.js opens the seeded dashboard preview via the shared gate key. Customer switch/fineprint hidden in admin mode. Build + Playwright verified incl. gate round-trip. |
...
| [x] | 112 | Dashboard — Analytics, Compliance, Federation | Auth & operator surfaces | Medium priority | Done. Analytics: calls/latency charts + drift chart on a FIXED 0-1 axis with shaded acceptable band (line() gains domain/bandRange opts, backwards-compatible) + 4-model legend. Compliance: case table sorted by SLA urgency w/ countdown chips (overdue/soon/ok). Federation: rounds table + Shapley bars sharing one participant color key. Build + Playwright verified (axis ticks 1.00-0.00, chip/bar colors equal). |
| [x] | 113 | Dashboard — admin tabs | Auth & operator surfaces | Medium priority | Done. Tenants: sortable columns (aria-sort) + plan/region filters w/ designed empty state. Approvals: rationale drawer gates the action buttons (EU AI Act risk-class rationale must be opened before Approve/Send back unlock). System: 3-node EU/US/MENA status map (degraded = amber, spotted spatially) + detail rows. Build + Playwright verified end-to-end. |
| [x] | 118 | Transactional email system | Onboarding & account lifecycle | Medium priority | Done (adapted: monorepo/packages target is purged; built as static artifacts). Shared 600px hand-coded base (tables, inline styles, bulletproof buttons, dark-mode meta + media query, hidden preheader) -> 5 variants under assets/email/ (receipt, dunning, kyc-status, alert-digest, federation-notice), EACH with a .txt twin; /emails/ fixtures preview renders all five in iframes. Build + Playwright verified. |
| [x] | 120 | Paper-Trading Sandbox | Marketplace expansion | Medium priority | Done. Sandbox tab on /dashboard/ (operator group): dashed sandbox treatment + amber SIMULATED banner, 4 trading models, multi-select comparison equity chart w/ SIMULATED watermark, per-model stat cards stamped SIMULATED, CSV export stamped in header, reset control re-seeds the run, sessionStorage persistence, empty state. Build + Playwright verified end-to-end. |
| [x] | 121 | Public Model Leaderboard | Marketplace expansion | Medium priority | Done. /leaderboard/ renders from committed _data/leaderboard.json (42 rows, 6 categories, generated from the registry): rank, key metric + direction, direction-adjusted improvement score (methodology note explains why raw figures are not comparable), Verified badge deep-linking to /compliance/?run_id=... (verifier gains query prefill + auto-run) vs dashed Self-attested styling, as-of stamps, filter-chip category tabs, no-JS renders all sections. Build + Playwright verified. |
| [x] | 124 | Federated Participant Console | Trust & federation surfaces | Medium priority | Done. Federation tab gains an Operator/Participant view toggle (sessionStorage-persisted). Participant view (Alpine Credit Union seat): node-agent status card w/ heartbeat, AWS Nitro attestation badge w/ expiry countdown (SGX/stub noted), 6-round participation & Shapley earnings table w/ USDC payout pills, data-lineage viewer (3 feature groups, 11 feature chips). Build + Playwright verified incl. toggle round-trip + persistence. |
| [x] | 97 | Sentiment from Filings (redesign) | Live model pages | Low priority | Done. "A detection, in place" filing-excerpt reader: sample 10-K-style passage with 7 inline sentiment marks (4 risk / 1 hedging / 2 confidence) + swatch legend + sample disclaimer + sentence tone. Honesty section kept verbatim; demo harness untouched. Build + Playwright verified. |
| [x] | 98 | Trade Finance Doc AI (redesign) | Live model pages | Low priority | Done. "What an extraction looks like" before/after panel: sample BoL excerpt (mono pre) left; 8-field normalised extraction table + UCP 600 art. 14(c) late-shipment discrepancy flag right; sample disclaimer. Compliance citations kept verbatim. Build + Playwright verified. |
| [x] | 105 | Research hub | Content & developer surface | Low priority | Done. Type indicator per card (Note plain / Methodology indigo / Whitepaper cyan badges); Methodology cards get brand-left-border, ~300-char excerpt + read-time chip; audit-log note typed Methodology; /compliance/ audit-log module gains a "How it works" callout cross-linking it. Build + Playwright verified. |
| [x] | 106 | Blog | Content & developer surface | Low priority | Done. Author card pinned under the lead (initials avatar w/ photo slot, name, title, one-line credential from _data/authors.yml keyed by post author); reading-progress kept; "Cited in this post" footer strip from page.cites (welcome post cites both research notes + trust page, all resolving). Build + Playwright verified. |
| [x] | 107 | About & Partnerships | Company & forms | Low priority | Done. /about/: 4-milestone timeline strip (founded, first federated round, first regulator engagement, 92-model catalogue) above the kept team grid. /partnerships/: 3-column Technology/Data/Distribution comparison table reusing the pricing-comparison pattern (5 rows: you are / bring / get / commercials / first step, with contact links), prose sections kept as detail. Build + Playwright verified. |
| [x] | 108 | Contact & Demo request | Company & forms | Low priority | Done. /contact/ opens with a two-card intent fork: "Book a demo" (urgent styling, links /demo/) vs "Ask us anything" (anchors the open form, next-business-day). ?topic= prefill kept AND extended: sales/enterprise suggest the demo fork, other topics suggest the form fork + scroll to it, select still pre-fills. Build + Playwright verified both directions. |
| [x] | 114 | 404 / error state | Auth & operator surfaces | Low priority | Done. 404 stays inside the system: standard header/footer (default layout), on-brand line "This model isn't in the catalogue — yet.", three quick links (Models/Docs/Contact) as badge-row; inline styles removed. Build + Playwright verified. |
| [x] | 122 | Data-Feed Catalog | Marketplace expansion | Low priority | Done. /data/ renders 6 feeds from _data/feeds.yml in the model-card style: freshness chip, jurisdiction badges, sample-field chips, expandable lineage detail linking the real model pages (+ audit-log lineage note), Subscribe gated EXACTLY like model subscriptions (same site.app.enabled branch + signup/contact URLs). Build + Playwright verified. |
