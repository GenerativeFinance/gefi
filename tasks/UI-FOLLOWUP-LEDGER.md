...
- [x] 314 — Learning service — done (NEW assets/js/app/learning-math.js holds progress, duration parsing, certificate issuance and the KPIs as pure functions, loaded by BOTH the learning page and the mock through the same vm shim. api/openapi/learning.yaml filled in (v0.2.0, 5 paths): catalog w/ q/type/level/segment filters and a `stats` block, NEW /learning/paths, enrolments GET/POST, progress PATCH (0–100, issuing the certificate in the same response so the client does not have to guess whether one was created), certificates, w/ CatalogItem/LearningPath/Enrollment/Certificate/LearningStats schemas and three notes at the top: CERTIFICATES ARE RECORDS, SAMPLE CREDENTIALS (a certificate here attests to nothing — not accredited, not verifiable, not a qualification) and that progress is in-memory. **Two invented figures removed:** (a) the Certificates KPI showed `completed + 1`, labelled "incl. onboarding", for a certificate NO RECORD BACKED — it is now the number of issued certificate records, and the test asserts it equals both the record count and the completion count; (b) "Hours Learned 24.5" was a hardcoded constant corresponding to nothing — hours are now summed from the durations of the items actually completed, which for the seeded catalogue is 65 minutes (1.1h), and the test asserts the figure moves by exactly the completed item's own duration. Durations like "1h 10min" and "2h" are parsed by one function rather than each surface guessing, and an unreadable duration contributes zero rather than a made-up number. Also: enrolling twice returns the existing enrolment rather than resetting progress (starting something you are part-way through must not lose your place), and re-completing an item does not mint a duplicate certificate. Verified: mock boots at 180 contract routes, all handled; curl checks (catalog stats, re-enrol preserving progress, 422 on progress 150, 40% issuing nothing, 100% issuing the record and moving the stats, re-completing issuing nothing new, 404 on an unknown item); Playwright 21/21 — KPIs identical in both modes AND countable from the cards on screen, the certificate count proven equal to the record count, hours proven to be the sum of the completed items' durations and rendered from the service's own figure, starting an item moving it to 20% and the service recording the enrolment, progress surviving a reload, completing issuing a certificate that the KPI follows, the hours KPI moving by exactly that item's duration, no duplicate on re-completion, out-of-range progress refused, and the whole flow working offline with the same arithmetic; smoke 21/21; fallback baseline — only /app/learning/ changed, by the test hooks plus the two corrected figures, inspected and accounted for; production build green w/ CSP untouched)
- [x] 315 — Reports & compliance engine — done (NEW assets/js/app/reports-math.js holds report naming, generate/definition validation, the compliance and risk roll-ups and the sample narrative as pure functions, loaded by BOTH the report pages and the mock through the same vm shim. api/openapi/reports.yaml filled in (v0.2.0, 11 paths): catalog w/ category/status filters returning the categories alongside the rows, generate (202 → pending), NEW GET one report, NEW SSE generation events, NEW /reports/{id}/content, schedules, NEW /reports/templates, custom definitions CRUD, and the compliance and risk registers — each now returning ITS TOTALS ALONGSIDE THE ROWS THEY WERE COUNTED FROM, so a client can check the headline against the table rather than trust it. Four notes stated at the top: HEADLINES ARE COUNTED, GENERATION IS ASYNCHRONOUS (a client must not present a pending report as available), SAMPLE DOCUMENTS (generated narrative over the demonstration dataset — no real client, position or transaction data, and not suitable for regulatory filing or client distribution; the compliance register records sample evaluations against named regimes and is NOT evidence of compliance with any of them), and DATES ARE ISO on the wire. **Three problems fixed:** (a) the page declared a report generated on a setTimeout OF ITS OWN, so it would have claimed success even if the service had failed or was never asked — generation now follows the server’s SSE event, with a single poll as a backstop, and the service refuses to serve content for a report that has not generated (409); (b) custom report definitions lived only in sessionStorage, so a saved report vanished with the tab — they now persist through the API and the test proves one written in one browser session is still there in a FRESH context, and the registry’s duplicate-name rule is applied on both sides in the same words; (c) the compliance and risk KPI strips each recomputed their own counts inline, which happened to be right but had nothing tying them to what the service would say — both now use the shared roll-up, which is also what the register endpoints return. Verified: mock boots at 184 contract routes, all handled; curl checks (compliance and risk totals reconciling with their own rows, filtering leaving the whole-register totals alone, 422 on an unknown category, 409 on content for a pending report, 422 on a definition with no fields); Playwright 24/24 — register KPIs identical in both modes AND countable from the cards on screen, the endpoint’s totals reconciling with its own rows, the page rendering the service’s figure, filtering not moving the totals, the total VaR proven to be the sum of the rows and the severity buckets proven to account for every row, a queued report reported as Pending then flipping to generated from the SERVER’S event, content refused while pending and served-and-marked-sample once generated, the same flow reaching generated offline with an IDENTICAL report name, a custom definition created, held by the service, and surviving into a fresh browser session, a duplicate refused with the same words, and every report page rendering dates in the one display format with no raw ISO or slash-separated dates leaking through; smoke 21/21; fallback baseline — the four pages this task touched changed by test hooks ONLY (six KPI hooks per register page, one root wrapper each on reports and custom-reports), with every rendered figure confirmed unchanged; production build green w/ CSP untouched)
- [x] 316 — Regulator portal services — done (NEW assets/js/app/regulator-math.js holds the audit register, the SLA clock and every overview aggregate as pure functions, loaded by BOTH the regulator pages and the mock through the same vm shim. api/openapi/regulator.yaml filled in (v0.2.0, 12 paths): NEW /regulator/overview, NEW /regulator/audits (the whole register, paged, reporting its real `total`), model and dataset audits, one audit, issues carrying their computed SLA state w/ severity and sla filters, raise, resolve (returning the RECOMPUTED overview so the caller need not guess how the headline moved), threads + NEW GET messages, standards, NEW /regulator/activity, and entity cross-reference resolution, w/ Overview/AuditRow/ModelAudit/DatasetAudit/Issue/RegThread/RegMessage/Standard/Activity schemas and three notes at the top: AGGREGATES ARE COUNTED FROM THE REGISTER, SLA CLOCKS RUN FROM A FIXED EPOCH, and SAMPLE SUPERVISION (nothing here is a regulatory determination, a supervisory action, or evidence of compliance with any named regime, and no real organisation is being supervised). **The central problem fixed:** EVERY overview figure was a STORED CONSTANT — 142 audits, 87.3% compliance, 15 standards, 94.2% completion, 4.8 days average resolution, 156 resolved — with no relationship to the handful of rows the registers actually held, and the audit-type breakdown was literally `audits30d × 0.41 / 0.29 / 0.19`: made-up proportions of a made-up total. The register now GENUINELY HOLDS its 142 audits (seeded deterministically, the same set on both sides), the listing pages through them, and the test pages to the END and counts 142 rows — so the headline is reconcilable rather than asserted. Every figure is derived and states the fraction it was taken over (completion is now "30 of 142 completed" = 21.1%, standards is the 4 the registry holds, and average resolution reads "—/nothing closed yet" until something closes). **Three further fixes:** (a) the type breakdown counted into a FIXED list of four categories, silently dropping the hand-written audits whose type sits outside it, so it did not sum to the total — it now counts whatever types the register really contains and the bars sum to 142; (b) the issue-severity bars were a fixed 30%/45% share of the flagged count, describing no actual issue — they are now real counts and sum to the open-issue total; (c) issues carried NO due date, so an SLA clock was impossible — the deadline now follows from severity and the date raised (critical 5 days, high 10, medium 20, low 30) against a FIXED portal epoch, so a screenshot today and one next month agree, and the list states the epoch it was computed against. Resolve and the communications composer now round-trip through the service. Verified: mock boots at 188 contract routes, all handled; curl checks (overview derived, register total, SLA states and filter, resolve returning the recomputed overview, 409 on a second resolve, entity links for MT/DS/CS and 404 for an unknown id and an unknown prefix); Playwright 24/24 — overview KPIs identical in both modes, the headline equal to the service figure, the register holding every audit the headline counts AND paging to exactly that many rows, both breakdowns accounting for every row with the on-page bars summing to the same figures, every open issue carrying a computed SLA state, the SLA filter returning exactly the overdue set, resolve moving the Overview SERVER-SIDE and the new figure surviving a reload, a second resolve refused, the composer’s message reaching the supervisory record and an empty one refused, valid cross-references resolving while an unknown id and prefix both 404 so the portal can render its designed not-found state, and the offline engine building an identical register; smoke 21/21; fallback baseline — the six regulator pages changed: four by the root wrapper alone, reg-issues by the wrapper plus the resolve hooks, and /app/regulator/ by the KPI hooks plus the derived figures replacing the constants, each inspected and accounted for; production build green w/ CSP untouched)
- [x] 317 — Notifications & alerts — done (NEW assets/js/app/notify-math.js holds the unread count, the alert-rule rules and the shape a notification takes as pure functions, loaded by BOTH the app shell and the mock through the same vm shim; NEW assets/js/app/notifications.js wires the topbar bell on EVERY app page (both layouts). api/openapi/notifications.yaml filled in (v0.2.0, 7 paths): notifications w/ unread/kind filters and the whole-set unread count alongside each page, mark-read returning the NEW count so the badge is not recomputed by guesswork, NEW /notifications/stream (SSE), alerts, alert-rules CRUD w/ NEW PATCH, delivery channels, w/ Notification/AlertRule/Channel schemas and two notes at the top: ONLY IN-APP DELIVERS (email and push are RECORDED PREFERENCES with no provider behind them — nothing is sent, nothing verified — and /delivery/channels reports `delivers:false` for both so a client can label them honestly) and ALERT RULES DO NOT FIRE ON THEIR OWN (a rule naming a metric the service does not track would never fire, so it is refused at creation rather than accepted and silently ignored). **The central problem fixed:** the bell was MARKUP ONLY — a permanently-lit red dot on a button that did nothing, on every app page. The dot now carries the real unread count and hides at zero, the button opens a dropdown listing the notifications w/ mark-all-read, and the count round-trips through the service. **Notifications are RAISED BY MUTATIONS, not invented:** the mock raises one when an order fills, a training job completes, a compliance issue resolves, a campaign reaches its goal, or a dataset publishes — five real hooks — and NEW /notifications/stream pushes each to every open tab, so placing an order pops the bell WITHOUT a reload. 205's Set Alert used to flip a sessionStorage flag and call it an alert; it now creates a real rule through POST /alert-rules watching the ONE number the insight actually carries (confidence below its current level), with the service writing the rule's description once so no two surfaces phrase it differently; duplicates 409. The marketing-site dashboard alert center was NOT touched — parity noted here as the task requires. Verified: mock boots at 190 contract routes, all handled; curl checks (unread count, order raising a notification, mark-read returning the new count, 422 on an untracked metric, 409 on a duplicate rule, channels honest, SSE delivering a raised notification); Playwright 19/19 — the badge carrying the service's unread count with the dot showing the number, offline rendering the same seeded state, the dropdown listing and labelling only-in-app plainly, placing an order pushing "Order filled" over SSE and popping the badge live, mark-all-read zeroing badge and dot with the service agreeing after reload, Set Alert creating a real rule that PATCH rewrites and DELETE removes, /delivery/channels naming in_app as the only deliverer, and the offline bell opening and clearing locally; smoke 21/21; fallback baseline — only the two insights pages changed (root wrapper + Set Alert hooks); the bell sits in the topbar, outside the .app-main region the baseline hashes; production build green w/ CSP untouched)
- [x] 318 — AI insights service — done (NEW assets/js/app/insights-math.js holds the four surfaces’ insight sets in ONE canonical shape — id, surface, title, body, sentiment, confidence, impact, generated — assembled from the canonical dataset (the portfolio set, the market block, the regulator insights and the provider trends each stored their own shape; this is now the single place that translates), loaded by BOTH the panels and the mock through the same vm shim. api/openapi/insights.yaml filled in (v0.2.0, 6 paths): /insights w/ a surface filter, NEW /insights/{id}, /insights/generate returning `source: seeded|claude` with every item labelled, /sentiment, /predictions, /narratives, w/ an Insight schema whose `generated` flag the contract makes MANDATORY to render — the label a reader uses to tell an authored insight from a generated one — and three notes at the top: SEEDED BY DEFAULT, THE OPTIONAL LIVE GENERATOR, NOT ADVICE. **The optional generator, exactly as specified:** behind GEFI_INSIGHTS_CLAUDE=1 + ANTHROPIC_API_KEY the mock calls the Claude API (model claude-sonnet-5, temperature 0), the prompt template lives in backend/mock/prompts/insights.txt (grounding rules: invent nothing beyond the supplied context, no advice, no price targets), output is STRICTLY validated against the insight schema (title ≤80, body ≤280, enum sentiment/impact, numeric 0–100 confidence — one bad item discards the whole batch), results carry generated:true, and ANY failure — no key, network, refusal, malformed output — falls back to the seeded set rather than surfacing an error; NO KEY IS IN THE REPO. The mock’s dispatcher gained async-handler support (settling a returned promise, treating rejection as a 500) since it previously serialised a pending promise as `{}`. The insights panel renders an "AI-generated" chip whenever generated:true, and the test asserts NOTHING carries the label while the generator is off. Verified: mock boots at 191 contract routes, all handled; curl checks (each surface’s list, 422 on an unknown surface, insight detail, generate returning source:seeded with every item false); the fallback promise tested DIRECTLY — a second server started with the flag ON and a deliberately invalid key returns source:seeded, not a 500; the live path with a real key is untestable here (no ANTHROPIC_API_KEY in the environment) and skipped with a note, as the task allows; Playwright 11/11 — insight pages live, the portfolio panel identical in both modes AND hydrated in the canonical shape from /insights?surface=portfolio, no AI-generated labels with the generator off, EVERY surface’s endpoint returning exactly what the shared module assembles, generate honestly labelled, the provider panel’s trends each backing an insight the endpoint returns, the regulator surface matching its panel, and 226’s market panel showing /sentiment’s own figures; smoke 21/21; fallback baseline — all 45 pages byte-identical (the changes are data-layer and a label path that is dormant offline); production build green w/ CSP untouched)
- [x] 319 — zKML proof pipeline — done (NEW assets/js/app/zkml-math.js holds the pipeline plan, the FNV-1a proof hash and the full EVENT TIMELINE as pure functions, loaded by BOTH page 231 and the mock through the same vm shim. **The bug this fixes:** the page and the mock each had their own plan() seeded from the SAME key but DRAWING IN A DIFFERENT ORDER — the page drew compile, create, then lanes; the mock drew lanes first — so the "same" verification produced different shard timings on each side; one function now, one draw order, one answer, and the page’s original draw order was kept verbatim so the offline fallback renders byte-identically. api/openapi/zkml.yaml filled in (v0.2.0, 6 paths): create (w/ model + 1–16 shard validation), history w/ a model filter, one verification, SSE events, proof lookup by hash, anchor stub, federation rounds, w/ ProofRecord/Verification schemas and two notes at the top: A SIMULATION OF THE PIPELINE, NOT THE PIPELINE (nothing cryptographic happens; the hash is FNV-1a over the inputs — a stable label, NOT a commitment, verifying nothing) and the stage semantics (prove lanes run in parallel: wall-clock follows the slowest lane, task time sums all of them; the record states both clocks). The SSE endpoint now walks the shared timeline tick by tick — zkml.stage frames as each of the five stages starts and finishes, zkml.log lines for the prover panel, zkml.shard_proved per lane, then one zkml.verified carrying the record — replacing a handler that emitted only shard events on a fixed cadence. The anchor stub says plainly that NOTHING IS WRITTEN TO ANY CHAIN and its tx id is a sample label. Page 231 delegates plan() and fnv1a to the module, records each run server-side at run start (data-zk-server-id/hash; "local" offline), and gains a Verification history panel persisted by the service in live mode w/ an honest offline empty state. Verified: mock boots at 191 contract routes, all handled; curl checks (create w/ the full record, same-inputs-same-hash, 422 on 0 shards and an unknown model, history by model, proof lookup, the SSE stream emitting 11 log + 10 stage + 4 shard + 1 verified events); Playwright 18/18 — the run walking all five stages w/ one lane per shard and the full log narrative, the run recorded server-side with the SERVER’S record hash equal to the PAGE’S animation hash (the divergence bug, proven fixed), the summary matching GET record on hash and both clocks, the same inputs yielding the same hash across runs AND a different shard count yielding a different one so the equality is not vacuous, the SSE stream walking EXACTLY the timeline the shared module builds (26 events) with the final frame carrying the matching record, the history panel listing the runs after a reload from the service with each row’s hash, and offline the same hash, an honest "local" marker and a byte-identical prover log; smoke 21/21 after updating the zkml check to the new record shape; fallback baseline — only /app/zkml/ changed, by the root wrapper plus the new history panel w/ its offline empty state, inspected and accounted for; production build green w/ CSP untouched)
- [x] 320 — Platform cross-cutting (API keys, rate limits, audit chain, search, i18n, GDPR) — done (NEW assets/js/app/platform-math.js holds the grouped search engine (models, datasets, docs, orders — groups ALWAYS present, ≤5 hits each, a `total` counted from the hits), key masking, the typed-confirm revoke rule, the FNV-1a-linked audit chain and the i18n bundles as pure functions, loaded by BOTH the app shell and the mock through the same vm shim; NEW assets/js/app/search.js wires the topbar search on EVERY app page (both layouts) — the second dead topbar control brought to life after 317's bell. api/openapi/platform.yaml filled in (v0.2.0, 11 paths): api-keys list/create/revoke, rate-limits, audit-chain run record + segment, search, i18n/{locale}, gdpr exports/deletions/requests/retention-jobs, w/ ApiKey/AuditLink/SearchHit schemas and four notes at the top: KEYS ARE SHOWN ONCE (`secret_once` rides ONLY the create response; every listing thereafter is masked to prefix+last-4, and the mock genuinely discards the secret — it cannot re-reveal), RATE-LIMIT HEADERS ON EVERY RESPONSE (X-RateLimit-Limit/-Remaining/-Reset on ALL mock responses, CORS-exposed so a browser client can actually read them — a header the client cannot read is a header that does not exist), THE CHAIN IS A SAMPLE (each link commits to its predecessor by hashing over the previous hash, `root` is the final hash and the record says plainly this is NOT a cryptographic commitment), GDPR REQUESTS ARE RECORDED STUBS (an export assembles and sends NOTHING; a deletion requires the literal typed DELETE and deletes nothing — both are recorded with status so the claim is bounded and inspectable). **The central problems fixed:** (a) the topbar search button was MARKUP ONLY on every app page — it now opens a panel whose grouped results come from /search live and the SAME shared engine offline; (b) revoking an API key was a client-side row removal anywhere it appeared — it now requires typing the key's LABEL exactly, refused otherwise (422) with the same words on both sides, and the key genuinely leaves the service's listing; (c) the trust-center verifier had NO endpoint behind it — GET /audit-chain/{run_id} now serves a seeded run record carrying verified:true and the root, and the segment endpoint slices the SAME chain. i18n ships en complete and es as an honest stub NAMING its fallback rather than silently mixing languages; an unknown locale 404s. **One bug found by the verification itself:** offline the fallback resolver answers /search but never sees the query string (fallbackFor strips it before matching), so the shell was rendering the resolver's EMPTY grouped result as if it were an answer — the shell now runs the shared engine locally whenever the response is a sample, which is also the honest architecture: offline search should not pretend to have asked anyone. Verified: mock boots at 195 contract routes, all handled; curl checks (grouped search w/ counted total, secret_once on create only, masked listing, 422 on a mismatched confirm, revoke removing the key, rate-limit headers on every response and exposed, run-42 record w/ root and link-committing chain, segment slice, en/es/404 locales, GDPR export + refused-then-confirmed deletion, requests list); Playwright 19/19 — live search grouped w/ every hit a real link, the OFFLINE panel identical for the same query (one shared engine), a symbol surfacing the orders group, the panel's count being the endpoint's own, create revealing the secret ONCE w/ the listing showing prefixes only forever after, the mismatched confirmation refused w/ the key surviving and the typed one revoking, X-RateLimit-* riding every response and CORS-readable from the browser, the run record carrying what the verifier reads w/ every link committing to its predecessor and the root equal to the last hash, the record stating plainly the chain is a sample, the segment slicing the same chain, en complete/es naming its fallback/unknown 404ing, a GDPR export recorded and honest, a deletion without the typed DELETE refused, and zero page errors in both modes; smoke 21/21; fallback baseline — ALL 45 pages byte-identical (the search panel lives in the topbar, outside the .app-main region the baseline hashes, and opens only on click); production build green w/ CSP untouched)
- [ ] 321 — Decision task: where the real backend runs — AWAITING OWNER (not self-answered; the contracts + mock make every UI wireable regardless of the choice — options and the four sub-decisions are laid out in the Task 321 section below)
