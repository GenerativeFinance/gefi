...
export default function ModelDetail() {
  ...
  const idParamRaw =
    ...
    (typeof window !== "undefined"
      ? (window.location.pathname.match(/\/(?:model|marketplace|models)\/(\d+)/) || [])[1]
      : undefined);
  ...
  // Check for special cases and redirect
  useEffect(() => {
    if (idParam === '8') {
  ...
  return (
    <Layout>
      <div className="container mx-auto p-6 max-w-7xl">
        {/* Minimal detail rendering so we don't change the rest of the page */}
        <div className="bg-card p-6 rounded-md">
          <h1 className="text-2xl font-bold mb-2">{(model as any)?.name || 'Model'}</h1>
          <p className="text-muted-foreground mb-4">{(model as any)?.description || 'No description available'}</p>
